"""Stage 1: surfacing relevance — the per-document, task-conditioned relevance states.

A relevance state is **not** a generic summary. Each one is written *for a specific
...
**final-round** states (``CorpusRelevanceState.states``); earlier rounds are kept only
for inspection.
"""
...
# Separator between the other documents' relevance states in the cross-conditioning block.
_OTHER_SEP = "\n\n---\n\n"


@dataclass
class CorpusRelevanceState:
    """Output of :func:`surface_relevance` — the corpus-wide relevance state.

    - ``states`` — the final-round per-document relevance states, aligned 1:1 with the
      input ``documents`` (``states[d]`` is the note document ``d`` contributes toward
      the task). This is what downstream stages consume.
    - ``rounds`` — every round's per-document states, in order: ``rounds[k]`` is round
      ``k+1`` (so ``rounds[-1] is states``). Kept for inspection of the refinement;
      not fed downstream.
    """
...
def render_relevance(relevance_states: list[str] | None, doc_ids: list[str] | None = None) -> str:
    """Render the final-round relevance states into the labeled block that stages 2
    and 3 embed as the corpus-wide relevance state.
    ...
    """
    if not relevance_states:
        return ""
    parts: list[str] = []
    for i, s in enumerate(relevance_states):
        text = (s or "").strip()
...
def _render_other_states(other_states: list[str]) -> str:
    """Concatenate the OTHER documents' relevance states for the cross-conditioning
    ...
    parts = [s.strip() for s in other_states if s and s.strip()]
...
def relevance_state(
    ...
    **llm_kwargs: Any,
    ...
    """Surface one ``document``'s relevance state for the task, given the OTHER
    documents' previous-round states (empty list in round 1).
    ...
    system_prompt = load_prompt(
        "relevance", version=prompt_version, task=task, other_states=_render_other_states(other_states)
    )
    return cast(
        str,
...
def surface_relevance(
    ...
    **llm_kwargs: Any,
    ...
    """Surface the corpus-wide relevance state over ``rounds`` synchronous rounds.

    Round 1 writes each document's state from the task + document alone; each later
    round rewrites every document given the *previous round's* states of the OTHER
    documents (never its own). Returns a :class:`CorpusRelevanceState` (`.states` =
    last-round per-document, `.rounds` = all rounds). ``rounds=1`` = independent
    round-1 only; ``rounds < 1`` = no relevance states at all (`.states` / `.rounds`
    empty — a deliberate "no-relevance" run). An empty ``documents`` → empty results.
    """
    # rounds < 1 = "no relevance states at all" (a deliberate rounds=0 re-run); an empty
    ...
    if rounds < 1 or not documents:
        return CorpusRelevanceState(states=[], rounds=[])
    ...
    def run_round(prev: list[str] | None, round_idx: int) -> list[str]:
        """Rewrite every document's relevance state in parallel. ``prev`` is the frozen
        ...
        def one(i: int, doc: str) -> str:
            # Others-only: document i sees the previous round's states of the OTHER
            ...
            return relevance_state(
                task=task, document=doc, other_states=others, model=model,
    ...
    return CorpusRelevanceState(states=all_rounds[-1], rounds=all_rounds)
